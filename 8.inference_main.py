...
def main():
    ...
    parser.add_argument('-sin', '--singer', type=str, default=fr"zsh", help='歌手姓名缩写')
    args = parser.parse_args()
    singer = args.singer  # 歌手名
    file_path = fr'G:\模型\so-vits-svc\{singer}'

    # 一定要设置的部分
    parser.add_argument('-m', '--model_path', type=str, default=fr"{file_path}\G.pth", help='模型路径')
    parser.add_argument('-c', '--config_path', type=str, default=fr"{file_path}\config.json", help='配置文件路径')
    parser.add_argument('-cl', '--clip', type=float, default=0, help='音频强制切片，默认0为自动切片，单位为秒/s')
    parser.add_argument('-n', '--clean_names', type=str, nargs='+', default=["1.wav"],
                        help='wav文件名列表，放在raw文件夹下')
    parser.add_argument('-t', '--trans', type=int, nargs='+', default=[0], help='音高调整，支持正负（半音）')
    parser.add_argument('-s', '--spk_list', type=str, nargs='+', default=[fr'{singer}'], help='合成目标说话人名称')

    # 可选项部分
    parser.add_argument('-a', '--auto_predict_f0', action='store_true', default=False,
                        help='语音转换自动预测音高，转换歌声时不要打开这个会严重跑调')
    parser.add_argument('-cm', '--cluster_model_path', type=str, default=fr"{file_path}\kmeans.pt",
                        help='聚类模型或特征检索索引路径，留空则自动设为各方案模型的默认路径，如果没有训练聚类或特征检索则随便填')
    parser.add_argument('-cr', '--cluster_infer_ratio', type=float, default=0.5,
    ...
    parser.add_argument('-shd', '--shallow_diffusion', action='store_true', default=True,
    ...
    # 浅扩散设置
    parser.add_argument('-dm', '--diffusion_model_path', type=str, default=fr"{file_path}\diffusion\model.pt",
                        help='扩散模型路径')
    parser.add_argument('-dc', '--diffusion_config_path', type=str, default=fr"{file_path}\diffusion\config.yaml",
                        help='扩散模型配置文件路径')
